...
// Fixed-width sibling of reg64bit, used for the narrower fields (control
// bits, register numbers, the instruction word) carried in the pipeline
// registers.  No parameters -- one module per width actually needed.
module reg1bit (q, d, en, reset, clk);
  ...
  mux2_1 m (.out(d_next), .i0(q), .i1(d), .sel(en));
  D_FF dff (.q(q), .d(d_next), .reset(reset), .clk(clk));
endmodule


module reg3bit (q, d, en, reset, clk);
  output logic [2:0] q;
  input  logic [2:0] d;
  input  logic en, reset, clk;
  logic [2:0] d_next;
  ...
  genvar i;
  generate
    for (i = 0; i < 3; i = i + 1) begin : reg_bit
        mux2_1 m (.out(d_next[i]), .i0(q[i]), .i1(d[i]), .sel(en));
        D_FF dff (.q(q[i]), .d(d_next[i]), .reset(reset), .clk(clk));
    end
  endgenerate
...
module reg5bit (q, d, en, reset, clk);
  output logic [4:0] q;
  input  logic [4:0] d;
  input  logic en, reset, clk;
  logic [4:0] d_next;
  ...
  genvar i;
  generate
    for (i = 0; i < 5; i = i + 1) begin : reg_bit
        mux2_1 m (.out(d_next[i]), .i0(q[i]), .i1(d[i]), .sel(en));
        D_FF dff (.q(q[i]), .d(d_next[i]), .reset(reset), .clk(clk));
    end
  endgenerate
...
module reg32bit (q, d, en, reset, clk);
  output logic [31:0] q;
  input  logic [31:0] d;
  input  logic en, reset, clk;
  logic [31:0] d_next;
