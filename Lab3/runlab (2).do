...
#
# Usage: launch ModelSim's GUI, then from its Transcript window (working
# directory = the folder with these files):
#     do runlab.do
# or from a command prompt: vsim -do runlab.do
# Do NOT use "vsim -c -do runlab.do" -- the -c flag runs console-only,
# with no GUI at all, so no wave window ever opens.
...
vlog -sv instructmem.sv    ;# provided -- edit its `define BENCHMARK` line to pick a program
...
# ---- Elaborate ----
# -novopt disables ModelSim's optimization entirely, guaranteeing every
# signal in the design (including internal ones like PC, the flags, and
# every register) is visible in the Objects browser and can be added to
# the wave -- no aliasing, no stripped "pass-through" wires. This matters
# more on ModelSim-Intel/Altera Starter Edition specifically, where
# "-voptargs=+acc" alone doesn't reliably prevent signals from being
# optimized away.
vsim -novopt work.cpu_testbench
