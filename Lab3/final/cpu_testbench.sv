...
module cpu_tb ();
  ...
  parameter NumCycles  = 800; // Covers every provided benchmark with margin --
                              // test11_Sort needs ~601 cycles, test12_Fibonacci
                              // ~236, everything else well under 100. All of them
                              // end in a self-loop (B to itself), so running past
                              // when they finish is harmless -- just idles there.
  ...
  initial begin
    ...
    $display("X31 = 0 (XZR, hardlogicd)");

    $display("\n---- Flags ----");
    $display("N = %b   V = %b", dut.N_flag, dut.O_flag);
