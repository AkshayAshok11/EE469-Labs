...
module cpu (
  ...
  assign ReadRegister1 = instr[9:5];   // Rn
  ...
  // PC update
  add64 pc_adder   (.in1(PC), .in2(64'd4), .sum(PCPlus4));
  add64 br_adder   (.in1(PC), .in2(ImmExt), .sum(BranchTarget));

  logic flags_neq, zero_taken, cond_taken;
  xor g_flags_neq  (flags_neq, N_flag, O_flag);
  and g_zero_taken (zero_taken, ZeroBranch, ALU_zero);
  ...
  mux64_2to1 pcbr_mux (
    .out(PCNext), .i0(PCBranchStage), .i1(ReadData2), .sel(RegBranch)
  );
endmodule
